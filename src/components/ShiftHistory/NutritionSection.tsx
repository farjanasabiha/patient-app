"use client";

import { Checkbox } from "@/components/ui/checkbox";
import { CollapsibleSection } from "../Common/sections/CollapsibleSection";

interface NutritionSectionProps {
  days: string[];
}

const NutritionSection: React.FC<NutritionSectionProps> = ({ days }) => {
  // Items for Meal Preparation
  const mealPreparationItems = ["Breakfast", "Lunch", "Dinner", "Snack"];

  // Items for Feeding
  const feedingItems = ["Self Feeding", "Assistance", "Complete Dependence"];

  // Items for Fluids
  const fluidsItems = ["Regular", "Thickened", "Amount"];

  // Items for Services Requested
  const servicesRequestedItems = [
    "Meal Setup",
    "Meal Preparation",
    "Feeding",
    "Special Diet (specify)",
  ];

  return (
    <CollapsibleSection
      title="Nutrition"
      className="mt-3"
    >
      <div className="flex flex-col gap-[12px]">
        {/* Meal Preparation Table */}
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="flex flex-col bg-white rounded-[10px] overflow-hidden border border-[#C7C7CC]">
              <div className="flex flex-row w-full border-b border-[#C7C7CC]">
                <div className="flex-[0_0_260px] flex items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]">
                  Meal Preparation
                </div>
                {days.map((day) => (
                  <div
                    key={day}
                    className="flex-1 flex justify-center items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]"
                  >
                    {day}
                  </div>
                ))}
              </div>

              {mealPreparationItems.map((item, index) => (
                <div
                  key={item}
                  className={`flex flex-row w-full ${
                    index !== mealPreparationItems.length - 1
                      ? "border-b border-[#C7C7CC]"
                      : ""
                  }`}
                >
                  <div className="flex-[0_0_260px] flex items-center p-3 font-poppins text-[14px] leading-[21px] text-[#8E8E93]">
                    {item}
                  </div>
                  {days.map((day) => (
                    <div
                      key={day}
                      className="flex-1 flex justify-center items-center p-3"
                    >
                      <Checkbox />
                    </div>
                  ))}
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Feeding Table */}
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="flex flex-col bg-white rounded-[10px] overflow-hidden border border-[#C7C7CC]">
              <div className="flex flex-row w-full border-b border-[#C7C7CC]">
                <div className="flex-[0_0_260px] flex items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]">
                  Feeding
                </div>
                {days.map((day) => (
                  <div
                    key={day}
                    className="flex-1 flex justify-center items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]"
                  >
                    {day}
                  </div>
                ))}
              </div>

              {feedingItems.map((item, index) => (
                <div
                  key={item}
                  className={`flex flex-row w-full ${
                    index !== feedingItems.length - 1
                      ? "border-b border-[#C7C7CC]"
                      : ""
                  }`}
                >
                  <div className="flex-[0_0_260px] flex items-center p-3 font-poppins text-[14px] leading-[21px] text-[#8E8E93]">
                    {item}
                  </div>
                  {days.map((day) => (
                    <div
                      key={day}
                      className="flex-1 flex justify-center items-center p-3"
                    >
                      <Checkbox />
                    </div>
                  ))}
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Fluids Table */}
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="flex flex-col bg-white rounded-[10px] overflow-hidden border border-[#C7C7CC]">
              <div className="flex flex-row w-full border-b border-[#C7C7CC]">
                <div className="flex-[0_0_260px] flex items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]">
                  Fluids
                </div>
                {days.map((day) => (
                  <div
                    key={day}
                    className="flex-1 flex justify-center items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]"
                  >
                    {day}
                  </div>
                ))}
              </div>

              {fluidsItems.map((item, index) => (
                <div
                  key={item}
                  className={`flex flex-row w-full ${
                    index !== fluidsItems.length - 1
                      ? "border-b border-[#C7C7CC]"
                      : ""
                  }`}
                >
                  <div className="flex-[0_0_260px] flex items-center p-3 font-poppins text-[14px] leading-[21px] text-[#8E8E93]">
                    {item}
                  </div>
                  {days.map((day) => (
                    <div
                      key={day}
                      className="flex-1 flex justify-center items-center p-3"
                    >
                      <Checkbox />
                    </div>
                  ))}
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Services Requested Table */}
        <div className="overflow-x-auto">
          <div className="min-w-[800px]">
            <div className="flex flex-col bg-white rounded-[10px] overflow-hidden border border-[#C7C7CC]">
              <div className="flex flex-row w-full border-b border-[#C7C7CC]">
                <div className="flex-[0_0_260px] flex items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]">
                  Services Requested
                </div>
                {days.map((day) => (
                  <div
                    key={day}
                    className="flex-1 flex justify-center items-center p-3 font-poppins font-semibold text-[14px] leading-[21px] text-[#1C1C1E]"
                  >
                    {day}
                  </div>
                ))}
              </div>

              {servicesRequestedItems.map((item, index) => (
                <div
                  key={item}
                  className={`flex flex-row w-full ${
                    index !== servicesRequestedItems.length - 1
                      ? "border-b border-[#C7C7CC]"
                      : ""
                  }`}
                >
                  <div className="flex-[0_0_260px] flex items-center p-3 font-poppins text-[14px] leading-[21px] text-[#8E8E93]">
                    {item}
                  </div>
                  {days.map((day) => (
                    <div
                      key={day}
                      className="flex-1 flex justify-center items-center p-3"
                    >
                      <Checkbox />
                    </div>
                  ))}
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </CollapsibleSection>
  );
};

export default NutritionSection;
