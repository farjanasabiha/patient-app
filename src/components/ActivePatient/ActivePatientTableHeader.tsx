...
interface ActivePatientTableHeaderProps {
...
const ActivePatientTableHeader: React.FC<ActivePatientTableHeaderProps> = ({
  onDivisionChange,
  onSearch,
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 w-full">
          ...
          {divisions.map((division, index) => (
            <button
              ...
              className={`
                ...
                ${index === divisions.length - 1 ? "rounded-r-[3px]" : ""}
                ${
      ...
      <div className="flex flex-row items-center px-3 py-[6px] gap-[10px] w-full sm:w-[264px] h-[36px] bg-white border border-[#C7C7CC] rounded-[4px] overflow-hidden focus-within:border-[#0071A4] focus-within:ring-1 focus-within:ring-[#0071A4] transition-all">
...
export default ActivePatientTableHeader;
